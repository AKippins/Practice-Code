...
main:  
       la $t1, A #loading address of Matrix A
...
PRINTM:
       beq $t5, $s0, PRINTC #check if it has gone through the whole row
       sll $t6,$s1,2 
...
MID:   
       addi $t2,$t2,1 #add to say it has made it through to Matrix B
...
SUBM:
       beq $t5, $s0, SUBC #check if it has gone through the whole row
       sll $t6,$s1,2 
       add $t7,$t1,$t6 #get address of value
       add $s2, $t2, $t6
       lw $t8, 0($t7)#a
